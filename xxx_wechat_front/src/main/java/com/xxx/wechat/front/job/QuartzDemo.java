...
public class QuartzDemo {
	...
	@Scheduled(cron = "0 0 0 */15 * ?") // 15天执行一次
	public void goBaiduAccessToken() {
		...
		for (int i = 0; i < 3; i++) {
			try {
				IBaiduApiAccessTokenService wechatTokenService = AppContextUtils
						.getBean(IBaiduApiAccessTokenService.class);
	...
	@Transactional
	@Scheduled(cron = "0 0/45 11 * * *") // 每天11点45分执行一次
	public void getLotteryDltHistory() throws Exception {
		...
		String lotteryDltUrl = "http://www.lottery.gov.cn/api/lottery_kj_detail_new.jspx?_ltype=4&_term=";
		LotteryDltHistoryApi lotteryDltHistoryApi = new LotteryDltHistoryApi();
		try {
			boolean b = true;
			ILotteryDltHistoryService lotteryDltHistoryService = AppContextUtils
					.getBean(ILotteryDltHistoryService.class);
			String lastTerm = lotteryDltHistoryService.getLastTerm();
			LotteryDltHistoryResponse lotteryDltHistoryResponse = null;
			LotteryDltHistory lotteryDltHistory = null;
			String term = "";
			if (StringUtils.isNotBlank(lastTerm)) {
				while (b) {
					lotteryDltHistoryResponse = lotteryDltHistoryApi.getLotteryDltHistory(lotteryDltUrl + term);
					lotteryDltHistory = convertLotteryDltHistory(lotteryDltHistoryResponse);
					if (Integer.valueOf(lotteryDltHistory.getTerm()) <= Integer.valueOf(lastTerm)) {
						b = false;
					} else {
						int result = lotteryDltHistoryService.add(lotteryDltHistory);
						if (result == 1) {
							List<LotteryDltHistoryDetail> lotteryDltHistoryDetailist = convertLotteryDltHistoryDetail(
									lotteryDltHistoryResponse);
							ILotteryDltHistoryDetailService lotteryDltHistoryDetailService = AppContextUtils
									.getBean(ILotteryDltHistoryDetailService.class);
							result = lotteryDltHistoryDetailService.add(lotteryDltHistoryDetailist);
						}
						term = String.valueOf(Integer.valueOf(lotteryDltHistory.getTerm()) - 1);
					}
				}
			} else {
				LotteryDltHistoryTermResponse lotteryDltHistoryTermResponse = lotteryDltHistoryApi
						.getLotteryDltTerm(lotteryDltTermUrl);
				logger.info("请求路径：" + lotteryDltTermUrl);
				...
				if (!CheckUtils.isNull(newTermList) && newTermList.size() > 0) {
					...
					for (int k = 0; k < newTermList.size(); k++) {
						newLotteryDltUrl = lotteryDltUrl + newTermList.get(k);
					...
					logger.info("成功插入大乐透数据" + newTermList.size() + "条");
				}
			}
	...
	private LotteryDltHistory convertLotteryDltHistory(LotteryDltHistoryResponse response) {
		...
		lotteryDltHistory.setLotteryDate(
				DateUtils.getTimestamp(DateUtils.parse(response.getLottery().getfTime(), DateUtils.YYYYMMDDHHMMSS)));
		...
		lotteryDltHistory.setTerm(response.getLottery().getTerm());
