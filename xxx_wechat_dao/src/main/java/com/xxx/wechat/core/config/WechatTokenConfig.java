...
public class WechatTokenConfig {
	...
	private static WechatTokenConfig wechatConfig;
	 ...
	 * @return
	 */
	public static WechatTokenConfig getInstance(){
		if(CheckUtils.isNull(wechatConfig)){
			wechatConfig = new WechatTokenConfig();
		}
		return wechatConfig;
	}

	private WechatTokenConfig() {
		appId = ConfigurationConfig.getInstance().getProperty(ConfigurationEnum.APPID);
		appSecret = ConfigurationConfig.getInstance().getProperty(ConfigurationEnum.APPSECRET);
	}
