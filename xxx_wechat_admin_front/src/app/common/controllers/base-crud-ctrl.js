...
function BaseCrudCtrl($window, $state, $stateParams, ApiSrv, vm, ctrlOpts) {
    ...
    vm.getDetail = function() {
        ApiSrv.exec(ctrlOpts.modelName + '/detail?id=' +$stateParams.id, null,{method:'GET'})
            .then(function(data) {
	...
	vm.getDetail2 = function() {
        ApiSrv.exec(ctrlOpts.modelName + '/detail?id=' +$stateParams.id+ "&sendFlag=" + $stateParams.sendFlag, null,{method:'GET'})
            .then(function(data) {
                if (ctrlOpts.afterGetDetail) {
                    vm.model = ctrlOpts.afterGetDetail(data);
                }  else {
               ...
               //$state.go("main.push.detail");
            });
    };
	
    vm.save = function() {
        if (ctrlOpts.validate) {
           let result = ctrlOpts.validate();
           if (result == false) {
           		return;
           }
        }    
