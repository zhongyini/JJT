...
import com.xxx.wechat.common.wechat.api.response.GetUserInfoListResponse;
import com.xxx.wechat.common.wechat.api.response.GetUserInfoResponse;
import com.xxx.wechat.common.wechat.api.response.GetUsersResponse;
...
public class UserAPI extends BaseApi {
    ...
    * @return 关注者信息对象
    */
   public GetUserInfoResponse getSNSUserInfo(String token, String openid) {
       BeanUtils.requireNonNull(openid, "openid is null");
       GetUserInfoResponse response;
       LOG.debug("获取关注者信息.....");
       String url = String.format(Constant.WechatUrl.GET_SNS_USERINFO, token, openid);
       BaseResponse r = executeGet(url);
       String resultJson = isSuccess(r.getErrcode()) ? r.getErrmsg() : r.toJsonString();
       logger.info("resultJson:"+resultJson);
       response = JSON.toJavaObject(JSONObject.parseObject(resultJson), GetUserInfoResponse.class);
//       response = JsonUtils.toBean(resultJson, GetUserInfoResponse.class);
       logger.info("response:"+response.getHeadimgurl());
       return response;
   }
   ...
   * @return 关注者信息对象
   */
  public GetUserInfoResponse getSNSUserInfoTwo(String token, String openid) throws ClientProtocolException, IOException, HttpResponseNullException {
	  	String method = "getSNSUserInfo";
		String requestUrl = String.format(Constant.WechatUrl.GET_SNS_USERINFO, token, openid);
		String result = HttpRequest.httpGetRequest(requestUrl);
		logInfoResult(method, result);
		GetUserInfoResponse response = JsonUtils.parseObject(result, GetUserInfoResponse.class);
