...
gulp.task('clean', function() {
    return gulp.src(released, { read: false })
...
gulp.task('image', function() {
    return gulp.src(paths.image)
        // .pipe(image())
        .pipe(gulp.dest(released + '/images'))
        .pipe(browserSync.stream());
})
...
function bundle() {
    ...
    //增加 "compact": false,解决打包时对js文件大小限制的问题
...
gulp.task('js-vendor', function() {
    ...
    stream.pipe(gulp.dest(released + '/js'));
...
gulp.task('cssfont', function() {
    return gulp.src([
        ...
        .pipe(gulp.dest(released + '/css/font'));
})
